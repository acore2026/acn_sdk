...
def main() -> None:
    # 1. 初始化 C：参加 B 发起的协作请求，对两个 track 使用不同接收模式
    agent_c = AcnSDK(agent_name="SpeakerDroneT3C2Tracks")

    # 2. 申请 C 的数字身份
    agent_c_ok, agent_c_id = agent_c.register_agent_info(
        AgentInfo(
            name="SpeakerDroneT3C2Tracks",
            owner="13800138222",
            description="SpeakerDroneModel, SN777777, C subscribes one track and fetches another",
            priority=4,
            metadata={"region": "CN", "role": "leaf_collaborator", "demo": "t3_two_tracks"},
        )
    )
    if not agent_c_ok:
    ...
    def agent_c_on_task_collaboration_request(payload: dict) -> None:
        print(f"[SpeakerDroneT3C2Tracks] on_task_collaboration_request payload={payload}")
    ...
    def agent_c_on_discover_result_received(payload: dict) -> None:
        print(f"[SpeakerDroneT3C2Tracks] on_discover_result_received payload={payload}")

    def agent_c_on_task_start_command(payload: dict) -> None:
        print(f"[SpeakerDroneT3C2Tracks] on_task_start_command payload={payload}")
        task_id = payload["task_id"]
        task_id_holder["value"] = task_id
        print(agent_c.request_task_execution(agent_c_id, payload["task_description"], task_id=task_id))

    def agent_c_on_terminate_task_received(payload: dict) -> None:
        print(f"[SpeakerDroneT3C2Tracks] on_terminate_task_received payload={payload}")
        ...
        print(f"[SpeakerDroneT3C2Tracks] request_terminate_task result={result}")

    def agent_c_on_subscribe_track_received(payload: dict) -> dict[str, str]:
        print(f"[SpeakerDroneT3C2Tracks] on_subscribe_track_received payload={payload}")
        track_modes: dict[str, str] = {}
        for track_info in payload.get("track_list", []):
            track = track_info["track"]
            if track == TOPIC_POSITION:
                mode = "fetch"
            elif track == TOPIC_STATUS:
                mode = "subscribe"
            else:
                mode = "subscribe"
            track_modes[f"{track_info['namespace']}::{track}"] = mode
        print(f"[SpeakerDroneT3C2Tracks] subscribe track modes={track_modes}")
        return track_modes
    ...
    agent_c.register_callbacks(
        ...
        on_subscribe_track_received=agent_c_on_subscribe_track_received,
    ...
    # 6. 等待 B 发起协作请求，并接收 A 发布的 Position/Status 话题
