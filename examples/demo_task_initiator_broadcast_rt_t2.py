...
def main() -> None:
    ...
    initiator_ok, initiator_id = initiator.register_agent_info(
        AgentInfo(
            name="AliceAgentT2",
            owner="13800138000",
            description="AgentModel-X, SN123456, multi-collaborator demo",
            priority=5,
    ...
    collaboration_phase: dict[str, int] = {"value": 0}
    ...
    def initiator_on_task_collaboration_request(payload: dict) -> None:
        print(f"[AliceAgentT2] on_task_collaboration_request payload={payload}")
    ...
    def initiator_on_discover_result_received(payload: dict) -> None:
        print(f"[AliceAgentT2] on_discover_result_received payload={payload}")
        ...
        phase = collaboration_phase["value"]
        if phase not in collaboration_started:
            raise RuntimeError(f"unexpected collaboration phase: {phase}")
        ...
        print(f"[AliceAgentT2] start_task_collaboration phase={phase} result={result}")
        collaboration_started[phase].set()

    def initiator_on_task_start_command(payload: dict) -> None:
        print(f"[AliceAgentT2] on_task_start_command payload={payload}")
    ...
    def initiator_on_terminate_task_received(payload: dict) -> None:
        print(f"[AliceAgentT2] on_terminate_task_received payload={payload}")
        task_id = payload.get("task_id")
        ...
        print(f"[AliceAgentT2] request_terminate_task result={result}")
    ...
    # 7. 先请求协作方1协助，协同建立后发送几条 track 消息
    collaboration_phase["value"] = 1
    print(initiator.request_task_collaboration(initiator_id, task_id, ["声光驱离"]))
    if not collaboration_started[1].wait(DEFAULT_WAIT_TIMEOUT_SECONDS):
        raise RuntimeError("Timed out waiting for collaborator 1 DISCOVER_RESULT.")
    ...
    # 8. 再请求协作方2协助，协同建立后继续发送几条 track 消息
    collaboration_phase["value"] = 2
    print(initiator.request_task_collaboration(initiator_id, task_id, ["空中喊话"]))
    if not collaboration_started[2].wait(DEFAULT_WAIT_TIMEOUT_SECONDS):
        raise RuntimeError("Timed out waiting for collaborator 2 DISCOVER_RESULT.")
    time.sleep(DEFAULT_SUBSCRIPTION_GRACE_PERIOD_SECONDS)
    report_task_info_messages(
        initiator,
        initiator_id,
