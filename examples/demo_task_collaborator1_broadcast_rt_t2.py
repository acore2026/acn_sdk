...
def main() -> None:
    ...
    collaborator_ok, collaborator_id = collaborator.register_agent_info(
        AgentInfo(
            name="RobotDogT2One",
            owner="13800138111",
            description="RobotDogModel, SN654321, collaborator 1",
            priority=4,
            metadata={"region": "CN", "role": "collaborator", "collaborator_index": 1},
        )
    )
    if not collaborator_ok:
        raise RuntimeError(collaborator_id)
    print(f"collaborator1_id={collaborator_id}")
    print(f"collaborator1 local agent_info={collaborator.query_agent_info(collaborator_id)}")
    print(f"collaborator1 owner agents={collaborator.query_agent_list('13800138111')}")

    task_id_holder: dict[str, str] = {"value": ""}

    # 3. 注册回调
    def collaborator_on_task_collaboration_request(payload: dict) -> None:
        print(f"[RobotDogT2One] on_task_collaboration_request payload={payload}")
    ...
    def collaborator_on_discover_result_received(payload: dict) -> None:
        print(f"[RobotDogT2One] on_discover_result_received payload={payload}")
    ...
    def collaborator_on_task_start_command(payload: dict) -> None:
        print(f"[RobotDogT2One] on_task_start_command payload={payload}")
        task_id = payload["task_id"]
        task_id_holder["value"] = task_id
        print(
    ...
    def collaborator_on_terminate_task_received(payload: dict) -> None:
        print(f"[RobotDogT2One] on_terminate_task_received payload={payload}")
        ...
        print(f"[RobotDogT2One] request_terminate_task result={result}")
    ...
    # 5. 入网认证
    print(f"collaborator1 join={collaborator.join_network(collaborator_id)}")
