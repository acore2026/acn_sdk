...
def main() -> None:
    # 1. 初始化 B：先参加 A 的任务，接收两个 track，再按 Position 触发对 C 的协作请求
    agent_b = AcnSDK(agent_name="BridgeRobotT3B2Tracks")

    # 2. 申请 B 的数字身份
    agent_b_ok, agent_b_id = agent_b.register_agent_info(
        AgentInfo(
            name="BridgeRobotT3B2Tracks",
            owner="13800138111",
            description="RobotDogModel, SN654321, B receives two tracks and requests C",
            priority=4,
            metadata={"region": "CN", "role": "bridge_collaborator", "demo": "t3_two_tracks"},
        )
    )
    if not agent_b_ok:
    ...
    position_message_count: dict[str, int] = {"value": 0}
    requested_c_collaboration = threading.Event()

    # 3. 注册回调
    def agent_b_on_task_collaboration_request(payload: dict) -> None:
        print(f"[BridgeRobotT3B2Tracks] on_task_collaboration_request payload={payload}")
    ...
    def agent_b_on_discover_result_received(payload: dict) -> None:
        print(f"[BridgeRobotT3B2Tracks] on_discover_result_received payload={payload}")
        ...
        result = agent_b.start_task_collaboration(
            ...
            task_id,
            "B发起请求，C参加并对两个track使用不同接收模式",
        )
        print(f"[BridgeRobotT3B2Tracks] start_task_collaboration B->C result={result}")
        b_to_c_started.set()

    def agent_b_on_task_start_command(payload: dict) -> None:
        print(f"[BridgeRobotT3B2Tracks] on_task_start_command payload={payload}")
        task_id = payload["task_id"]
        task_id_holder["value"] = task_id
        print(agent_b.request_task_execution(agent_b_id, payload["task_description"], task_id=task_id))

    def agent_b_on_terminate_task_received(payload: dict) -> None:
        print(f"[BridgeRobotT3B2Tracks] on_terminate_task_received payload={payload}")
        ...
        print(f"[BridgeRobotT3B2Tracks] request_terminate_task result={result}")

    def agent_b_on_subscribe_track_received(payload: dict) -> dict[str, str]:
        print(f"[BridgeRobotT3B2Tracks] on_subscribe_track_received payload={payload}")
        track_modes = {
            f"{track_info['namespace']}::{track_info['track']}": "subscribe"
            for track_info in payload.get("track_list", [])
        }
        print(f"[BridgeRobotT3B2Tracks] subscribe track modes={track_modes}")
        return track_modes
    ...
    def agent_b_on_message_received(namespace: str, track: str, payload: bytes) -> None:
        ...
        if track != TOPIC_POSITION:
            return

        position_message_count["value"] += 1
        print(f"[BridgeRobotT3B2Tracks] position_message_count={position_message_count['value']}")
        if position_message_count["value"] != 3 or requested_c_collaboration.is_set():
            return
        ...
        print(agent_b.request_task_collaboration(agent_b_id, task_id, ["空中喊话支援"]))
    ...
    agent_b.register_callbacks(
        ...
        on_subscribe_track_received=agent_b_on_subscribe_track_received,
    ...
    # 6. 等待 A 发起协作、B 收到第 3 条 Position 后发起 C 协作、以及 A 广播终止任务
