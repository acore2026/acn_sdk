...
class MOQPublisher:
    ...
    def __init__(self, relay_host: str, relay_port: int):
        ...
        self._on_publication_rejected: Optional[
    ...
    async def connect(self, agent_id: Optional[str] = None) -> bool:
        ...
        try:
            ...
            self._session = MOQSession(
                session_id=f"pub-{id(self)}", role=Role.PUBLISHER
            )
            self._session.set_send_callback(self._send_data)
            self._session.set_handlers(on_publish=self._handle_publish_response)
    ...
    async def publish(self, track_name: FullTrackName) -> bool:
        ...
        try:
            ...
            await asyncio.wait_for(
                self._publish_waiters[request_id].wait(), timeout=5.0
            )
    ...
    async def send_object(self, track_name: FullTrackName, obj: PublishedObject):
        ...
        # Update activity timestamp to prevent unnecessary heartbeats
    ...
    async def _send_datagram_object(self, track_alias: int, obj: PublishedObject):
        ...
        datagram = ObjectDatagram(header=header, payload=obj.payload)

        data = datagram.encode()
        await self._client.send_datagram(data)

        logger.debug(
            f"Sent datagram object: group={obj.group_id}, object={obj.object_id}"
        )
    ...
    async def _send_stream_object(self, track_alias: int, obj: PublishedObject):
        ...
        if stream_key not in self._streams:
            ...
            header_data = (
                VarInt.encode(StreamType.SUBGROUP_HEADER) + subgroup_header.encode()
            )
        ...
        # Send object
        subgroup_obj = SubgroupObject(object_id=obj.object_id, payload=obj.payload)

        await self._client.send_stream_data(stream_id, subgroup_obj.encode())

        logger.debug(
            f"Sent stream object: group={obj.group_id}, object={obj.object_id}"
        )

    async def close_subgroup_stream(
        self, track_alias: int, group_id: int, subgroup_id: int
    ):
        ...
        if stream_key in self._streams:
            ...
            await self._client.send_stream_data(
                stream_id, end_obj.encode(), end_stream=True
            )

            del self._streams[stream_key]
            logger.debug(
                f"Closed subgroup stream: track={track_alias}, group={group_id}, subgroup={subgroup_id}"
            )
    ...
    async def _handle_stream_data(self, protocol, data: StreamData):
        ...
        logger.debug(
            f"Received stream data: stream_id={data.stream_id}, length={len(data.data)}"
        )
    ...
    async def _handle_close(self, protocol, error_code: int, reason: str):
        ...
        # Clean up local resources without re-entering the transport close path.
        if self._heartbeat_task:
            self._heartbeat_task.cancel()
            self._heartbeat_task = None
        self._cleanup_all_streams()
    ...
    def _cleanup_all_streams(self):
        ...
        if self._streams:
            logger.debug(f"Cleaning up {len(self._streams)} streams")
            self._streams.clear()

    def _cleanup_all_waiters(self):
        """Clean up all pending publish waiters."""
        if self._publish_waiters:
            logger.debug(f"Cleaning up {len(self._publish_waiters)} waiters")
            # Set all waiters to signal completion (with failure)
            for request_id, waiter in list(self._publish_waiters.items()):
                try:
                    if not waiter.is_set():
                        waiter.set()
                except Exception as e:
                    logger.warning(
                        f"Error signaling waiter for request_id={request_id}: {e}"
                    )
